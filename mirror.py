...
class MirroredContent(object):
    ...
    def fetch_and_store(key_name, base_url, translated_address, mirrored_url):
        ...
        try:
            response = urllib2.urlopen(mirrored_url)
...
class HomeHandler(BaseHandler):
    ...
    def post(self):
        form_url = self.get_argument('url')
        if form_url:
            # Accept URLs that still have a leading 'http://'
            inputted_url = urllib2.unquote(form_url)
...
class MirrorHandler(BaseHandler):
    ...
    def get(self, base_url):
        ...
        logging.debug('User-Agent = "%s", Referrer = "%s"',
                      ...
                      getattr(self.request, 'referer', '-'))
        logging.debug('Base_url = "%s", url = "%s"', base_url, self.request.uri)

        translated_address = self.get_relative_uri()[1:]  # remove leading /
        ...
        if content is None:
            ...
            content, response = MirroredContent.fetch_and_store(key_name, base_url,
                                                      translated_address,
                                                      mirrored_url)
        if content is None:
